...
def main():
    ...
    parser.add_argument("--KB", action="store_true",
                    help="Use KB parameters rather than Jancu")
    ...
    #Determine which slater-koster parameters to add
    #print(dir(cfg.sk_AB))
    set_suffix = "KB" if args.KB else "jancu"
    print(f"Selecting parameters with suffix: {set_suffix}")
    ...
    for b in range(nb):
        if b % 2 == 0:  # even index
            plt.plot(kd, Evals[:, b], "black", lw=1)
        else:           # odd index
            plt.plot(kd, Evals[:, b], "black", lw=1)
    ...
    plt.title(f"{name} Bandstructure")
    plt.tight_layout()
    plt.grid(True)
